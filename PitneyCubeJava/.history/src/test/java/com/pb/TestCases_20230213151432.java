...
public class TestCases {
	...
	public void testVersion() {
		...
		doReturn(true).when(pitneycube).GetScaleVersion(version);
		CubeResult ex = new CubeResult(true, 0, null);
	...
	public void testFailVersion() {
		byte[] version = new byte[48];
		when(pitneycube.GetScaleWeight(version)).thenReturn(0);
...
{
	when(pitneycube.CheckStatus()).thenReturn(0);
	CubeResult ex= new CubeResult(true,0,null);
...
{
	when(pitneycube.CheckStatus()).thenReturn(1);
	CubeResult ex= new CubeResult(true,0,No Paper);
	CubeResult ac=ps.checkStatus();
	assertEquals(ex.toString(), ac.toString());
}
}
