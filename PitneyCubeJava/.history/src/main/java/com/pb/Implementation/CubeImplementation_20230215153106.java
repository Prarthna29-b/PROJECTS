...
public class CubeImplementation {
	...
	public CubeResult connectPrinter(int i, String s, int j, int k, int l, int m) {
		CubeResult	pc = new CubeResult(false, null, null);
	...
	public CubeResult getScaleVersion() {
		...
		boolean res = cubeDLL.GetScaleVersion(CDRIVES);
		if(res)
		{
		pc.setResult(true);
		String w = new String(version, StandardCharsets.UTF_8);
		pc.setValue(w);
		}
		return pc;
	}
	...
	public CubeResult getScaleGravityInformation() {
		...
		boolean res = cubeDLL.GetScaleGravityInformation(gravity);
		if(res) {
		String w = new String(gravity, StandardCharsets.UTF_8);
		pc.setValue(w);
		pc.setResult(res);
		}
		return pc;
	...
	public CubeResult getRollType() {
		...
		boolean res = cubeDLL.GetRollType(rollType);
		if(res){
		String w = new String(rollType, StandardCharsets.UTF_8);
		pc.setValue(w);
		pc.setResult(res);
		}
