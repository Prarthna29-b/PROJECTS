...
public class CubeImplementation {
	...
	public String GetScaleGravityInformation(CubeResult pc) {
		...
		boolean gi=cubeDLL.GetScaleGravityInformation(gravity);
		pc.setValue(gi);
		return pc;	
	}*/
...
public 	CubeResult getProductNumber(CubeResult pc)
{
	byte[] productNumber  = new byte[48];
	 Long res=cubeDLL. GetPB_ProductNumber(productNumber);
	 int res1= (int) res;
	 pc.setValue( res);
	return pc;
 }
 public	String getSerialNumber(CubeResult pc)
{
	byte[] serialNumber  = new byte[48];
	Long  res = cubeDLL. GetPB_SerialNumber(serialNumber);
	pc.setValue(res);
	return pc;
}
